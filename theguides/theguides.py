...
class GuidesCommittee(commands.Cog):
    ...
    async def on_thread_close(self, thread, closer, silent, delete_channel,
        ...
        if thread.recipient.id == closer.id:
            ...
            month = await count_user_tickets_this_month(
                self.pool, closer.id, 30)
            print(f"Added 1 ticket to {closer} ({closer.id}")

            try:
                await closer.send(
                    f"Congratulations on closing your ticket {closer}. This is you ticket number `{week}` this week and your ticket number `{month}` this month."
                )
            except discord.errors.Forbidden:
                pass
